const BASE="https://raw.githubusercontent.com/POIenexis/POI-zoeker/main/",VER="5.2";
...
function links(i){let lat=i.lat,lon=i.lon,enc=encodeURIComponent([i.street,i.house,i.zip,i.city].filter(Boolean).join(" "));return{apple:`https://maps.apple.com/?daddr=${lat},${lon}&dirflg=d`,google:`https://www.google.com/maps/dir/?api=1&destination=${lat},${lon}`,waze:`https://waze.com/ul?ll=${lat}%2C${lon}&navigate=yes`,tomtom:`tomtomgo://x-callback-url/navigate?destination=${lat},${lon}`,geo:`geo-navigation://directions?destination=${lat}%2C${lon}`,androidgeo:`geo:${lat},${lon}?q=${lat},${lon}(${enc})`}}
async function shareDestination(i){const L=links(i),ios=/iPhone|iPad|iPod/.test(navigator.userAgent),text=[i.code||i.name||"Locatie",[i.street,i.house,i.zip,i.city].filter(Boolean).join(" "),`${i.lat},${i.lon}`].filter(Boolean).join("\n"),url=ios?L.apple:L.google;if(navigator.share){try{await navigator.share({title:"Navigeren naar locatie",text,url});return}catch(e){if(e.name==="AbortError")return}}try{await navigator.clipboard.writeText(text+"\n"+url);toast("Bestemming gekopieerd — open je navigatie-app")}catch(e){toast("Delen wordt niet ondersteund")}}
async function shareToTesla(i){toast("Kies Tesla in het deelmenu");setTimeout(()=>shareDestination(i),180)}

function navURL(i){let pref=localStorage.getItem("navPref")||"auto",L=links(i),ios=/iPhone|iPad|iPod/.test(navigator.userAgent);if(["flits","geo"].includes(pref)){pref="auto";localStorage.setItem("navPref","auto")}
if(pref=="auto")return ios?L.apple:L.google;if(pref=="tesla")return "#tesla-share";return L[pref]||L.google}
function detail(i){if(!i)return;recent(i);let fav=get("favs").includes(i.k),L=links(i);$("#detail").innerHTML=`<div class="badge">${i.type}</div><h2>${esc(i.code||i.name||"POI")}</h2><p class="meta">${esc(i.name)}<br>${esc([i.street,i.house,i.zip,i.city].filter(Boolean).join(" "))}<br>${i.lat.toFixed(6)}, ${i.lon.toFixed(6)}</p>${(localStorage.getItem("navPref")||"auto")==="tesla"?'<button class="navbtn primary tesla-primary">NAVIGEER</button>':`<a class="navbtn primary" href="${navURL(i)}">NAVIGEER</a>`}<details class="navchoices"><summary class="navbtn minor"><span class="navchoice-closed">Andere navigatie-app</span><span class="navchoice-open">↑ Navigatiekeuze sluiten</span></summary><a class="navbtn minor" href="${L.apple}">Apple Maps</a><a class="navbtn minor" href="${L.google}">Google Maps</a><a class="navbtn minor" href="${L.waze}">Waze</a><a class="navbtn minor" href="${L.tomtom}">TomTom GO</a><button class="navbtn minor tesla-nav">Tesla</button><button class="navbtn minor share-nav">Deel naar een andere app…</button><div class="nav-share-hint">Bijv. Flitsmeister of een andere geschikte navigatie-app</div></details><div class="actions"><button id="share">↗ Delen</button><button id="copy">⧉ Kopiëren</button></div><button class="navbtn secondary" id="fav">${fav?"★ Verwijder favoriet":"☆ Favoriet"}</button>`;$("#share").onclick=()=>share(i);$("#copy").onclick=()=>navigator.clipboard.writeText(`${i.code} ${i.name}\n${[i.street,i.house,i.zip,i.city].filter(Boolean).join(" ")}\n${i.lat}, ${i.lon}`).then(()=>toast("Gekopieerd"));$("#fav").onclick=()=>{let a=get("favs");a=fav?a.filter(x=>x!==i.k):[i.k,...a.filter(x=>x!==i.k)].slice(0,50);save("favs",a);toast(fav?"Favoriet verwijderd":"Favoriet toegevoegd");$("#detailDlg").close();render()};$(".share-nav")?.addEventListener("click",()=>shareDestination(i));$(".tesla-nav")?.addEventListener("click",()=>shareToTesla(i));$(".tesla-primary")?.addEventListener("click",()=>shareToTesla(i));$("#detailDlg").showModal()}
async function share(i){let text=`${i.code} ${i.name}\n${[i.street,i.house,i.zip,i.city].filter(Boolean).join(" ")}\n${links(i).google}`;if(navigator.share)await navigator.share({title:"StationZoeker",text});else navigator.clipboard.writeText(text).then(()=>toast("Deellink gekopieerd"))}
...
function setPos(p,silent=false,force=false){
 ...
 P=next;localStorage.setItem("locationEnabled","1");if(shouldRender)geoVersion++;

 if(shouldRender){lastRenderPos={latitude:P.latitude,longitude:P.longitude};render()}
 if(!silent)toast("Locatie actief")
}
function locate(fromSettings=false){if(!navigator.geolocation)return toast("Locatie niet beschikbaar");$("#status").textContent="Locatie bepalen…";navigator.geolocation.getCurrentPosition(p=>{mode="normal";setPos(p,false,true);if(fromSettings)$("#settingsDlg").close()},e=>{toast(e.code===1?"Locatietoegang geblokkeerd — controleer Safari-instellingen":"Locatie kon niet worden bepaald");render()},{enableHighAccuracy:true,timeout:12000,maximumAge:30000})}
...
q.oninput=()=>{mode="normal";clearTimeout(timer);timer=setTimeout(render,100)};$("#clear").onclick=()=>{q.value="";mode="normal";render()};$("#near").onclick=locate;$("#recentBtn").onclick=()=>{mode="recent";q.value="";render()};$("#favBtn").onclick=()=>{mode="fav";q.value="";render()};
